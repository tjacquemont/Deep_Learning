...
def extract_inclusion_num(patient_id: str) -> float:
    """
    Extrait le numéro d'inclusion depuis un identifiant 'AA-00' (ex: 'JD-03' -> 3).
...
def formater_donnees_excel(chemin_fichier, chemin_fichier_irm_csv, anonymisation=False, chemin_sortie_csv=None):
    """
    ...
    df = df.rename(columns={
        ...
        "Delai follow up en mois" : "Delai_follow_up_en_mois",
   ...
   # Find the index of "Remarques_suivi" and drop columns to its right
    ...
    print("Semicolons have been removed from the 'Numero' column.")
    
    df["Sexe"] = df["Sexe"].replace({'F': 0.0, 'H': 1.0})

    # Traiter la colonne "Lateralite_D"
    df["Lateralite_D"] = df["Lateralite_D"].replace({1: 0.0, 2: 1.0})

    # Traiter la colonne "Type_Ischemic"
    df["Type_Ischemic"] = df["Type_Ischemic"].replace({2: 0.0, 1 : 1.0})

    # Traiter la colonne "COTE_D"
    df["COTE_D"] = df["COTE_D"].replace({1: 0.0, 2: 1.0})
    
    for columns in df.columns:
        if columns != 'Nom':
            df[columns] = df[columns].astype(str).replace('POLION', str(np.nan), regex=False)
...
#        df = df.drop(columns=["TTT_0rien_1TIV_2TM_3_comb"])
    ...
    if "Date_AVC" in df.columns:
        ...
        new_irm_cols = [
            "Date_Diffusion_Raw", "IRM_Diffusion_Raw", "IRM_Diffusion_Norm", "IRM_Diffusion_Norm_to_MNI",
            "IRM_B0_Raw", "IRM_B0_Norm", "IRM_B0_Norm_to_MNI", "IRM_FLAIR_Raw",
            "Lesion_Mask_Norm", "Lesion_Mask_Norm_to_MNI", "ADC_Raw", "ADC_Norm",
            "ADC_Norm_to_MNI"
            ]
...
def update_image_db_from_prep(Image_Database_df: pd.DataFrame,
                              PREP_database_df: pd.DataFrame,
                              key: str = "numero",
                              interactive: bool = False,
                              columns_to_check=None,
                              ):
    """
    1) Identifie le patient limite = max(Image_Database_df[key])
    2) Pour les patients <= limite:
       - si Image a NaN ET PREP a une valeur non-NaN, propose une mise à jour (interactive)
    3) Pour les patients > limite:
       - ajoute les lignes PREP manquantes à Image
    """

    img = Image_Database_df.copy()
    prep = PREP_database_df.copy()

    if key not in img.columns or key not in prep.columns:
        raise ValueError(f"La colonne clé '{key}' doit exister dans les deux DataFrames.")

    # Inclusion number (pour définir la limite et comparer)
    img["_incl_num"] = img[key].apply(extract_inclusion_num)
    prep["_incl_num"] = prep[key].apply(extract_inclusion_num)

    if img["_incl_num"].dropna().empty:
        raise ValueError(f"Impossible de déterminer la limite: aucun '{key}' de Image_Database_df n'a un format '*-<digits>'.")

    # 1) Patient limite = max inclusion dans Image
    incl_limite = int(img["_incl_num"].dropna().max())
    print(f"Inclusion limite (max dans Image_Database_df) = {incl_limite}")

    # Colonnes à comparer = communes (hors key et _incl_num)
    common_cols = [c for c in img.columns if c in prep.columns]
    common_cols = [c for c in common_cols if c not in (key, "_incl_num")]

    if columns_to_check is None:
        columns_to_check = common_cols
    else:
        columns_to_check = [c for c in columns_to_check if c in common_cols]

    # Indexation par patient id exact (AA-00)
    img_idx = img.set_index(key, drop=False)
    prep_idx = prep.set_index(key, drop=False)

    # 2) Avant / jusqu'à la limite: mise à jour conditionnelle (NaN -> valeur PREP)
    shared_patients = img_idx.index.intersection(prep_idx.index)

    updates_done, updates_skipped = 0, 0

    for pid in shared_patients:
        # inclusion du patient (on prend celle de l'image, sinon celle de PREP)
        incl = img_idx.at[pid, "_incl_num"]
        if pd.isna(incl):
            incl = prep_idx.at[pid, "_incl_num"]

        if pd.isna(incl) or int(incl) > incl_limite:
            continue  # pas dans la zone "avant limite"

        for col in columns_to_check:
            old_val = img_idx.at[pid, col]
            new_val = prep_idx.at[pid, col]

            if pd.isna(old_val) and (not pd.isna(new_val)) and (not new_val=='nan'):
                if interactive:
                    print("\n" + "-"*70)
                    print(f"Patient {key}={pid} | Inclusion={int(incl)} | Colonne: {col}")
                    print(f"Image_Database_df : {old_val}")
                    print(f"PREP_database_df  : {new_val}")
                    ans = input("Intégrer dans Image_Database_df ? (y/n) : ").strip().lower()
                    if ans in ("y", "yes", "o", "oui"):
                        img_idx.at[pid, col] = new_val
                        updates_done += 1
                        print("✅ Mis à jour.")
                    elif ans in ("q", "quit"):
                        break
    ...
    # 3) Au-delà de la limite: ajouter les lignes PREP (même si pas dans Image)
    prep_after = prep[prep["_incl_num"].notna() & (prep["_incl_num"] > incl_limite)].copy()

    # On n'ajoute que les patients pas déjà présents dans Image (même ID exact)
    missing_ids = [pid for pid in prep_after[key].tolist() if pid not in img_idx.index]

    if missing_ids:
        rows_to_add = prep_idx.loc[missing_ids].copy()
        # aligne les colonnes sur Image, complète avec NaN si besoin
        rows_to_add = rows_to_add.reindex(columns=img_idx.reset_index(drop=True).columns).replace('nan','')
        updated = pd.concat([img_idx.reset_index(drop=True), rows_to_add.reset_index(drop=True)], ignore_index=True)
    else:
        updated = img_idx.reset_index(drop=True)

    # Nettoyage colonne technique
    updated = updated.drop(columns=["_incl_num"], errors="ignore")

    print("\n" + "="*70)
    print(f"Mises à jour appliquées: {updates_done}")
    print(f"Mises à jour ignorées : {updates_skipped}")
    print(f"Nouveaux patients ajoutés (incl > {incl_limite}): {len(missing_ids)}")
    print("="*70)

    return updated

##############################################################################
PREP_Path = '/home/thomas.jacquemont/Test/PREP_AVC/1_Database_Update/Table/20251217/Base PREP_a jour_2025.07.03.xlsx'
Image_Database_path = '/home/thomas.jacquemont/Test/PREP_AVC/1_Database_Update/Table/20251217/Base_PREP_Thomas.csv'
Updated_Image_Database_Path = '/home/thomas.jacquemont/Test/PREP_AVC/1_Database_Update/Table/20251217/Updated_Base_PREP_Thomas.csv'

Prep_Formated = formater_donnees_excel(PREP_Path, Image_Database_path)
Image_Database = pd.read_csv(Image_Database_path)
Image_Database_updated = update_image_db_from_prep(Image_Database, Prep_Formated)
Image_Database_updated.to_csv(Updated_Image_Database_Path, index=False)

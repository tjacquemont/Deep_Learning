...
class CustomDataset(Dataset):
    """Dataset personnalisé pour charger les données NumPy."""
    def __init__(self, X, y, transform=None):
        self.X = torch.tensor(X, dtype=torch.float32)
        self.y = torch.tensor(y, dtype=torch.float32).unsqueeze(1) # Assure que y a la forme (N, 1)
...
def stratified_shuffle_split(X, y, test_size=0.2, random_state=None):
    ...
    Args:
        X (np.array): Les caractéristiques (features) de votre jeu de données.
        ...
        random_state (int, optional): Graine pour la reproductibilité du mélange.
...
def data_augmentation_function_median_flip(X_train, y_train, working_dir='working_dir'):
    """
    ...
    Args:
        X_train (np.array): Données d'entraînement (N, C, Y, X, Z).
    ...
    for i in range(len(X_train)):
        ...
        flipped_image = np.flip(original_image, axis=1).copy() # Utilisez .copy() pour éviter les problèmes de vue

        augmented_X.append(flipped_image)
        augmented_y.append(original_label)

        # Visualisation des coupes médianes (axe Z - l'axe 1 après la forme actuelle)
...
def performance_visualizer(history, dataset_path, model_name, epochs, y_true, y_pred, working_dir):
    """
...
def build_and_train_model(train_loader, val_loader, model, epochs=50, learning_rate=0.001, loss_fn=nn.BCEWithLogitsLoss(), regularizers_l2=0, metric_list=['accuracy'], early_stop = False, device='cpu', work_dir='/home/thomas.jacquemont/cenir_thomas.jacquemont/PREP_AVC/Results/Deep_Learning/Debugs'):
    """
    ...
    for epoch in range(epochs):
        ...
        train_accuracy = train_corrects / total_train if total_train > 0 else 0
        val_accuracy = val_corrects / total_val if total_val > 0 else 0
...
def performances_saving(csv_path, history, model_name, data_augmentation, batch_size, learning_rate, dropout, regularizers_l2, epochs):
...
def training_pipeline(pickle_path, model_function, model_name, working_dir, performance_csv_path=None, data_augmentation=True, epochs=50, loss_fn=nn.BCEWithLogitsLoss(), metric_list=['accuracy'], batch_size=16, random_state=2301, dropout=0.5, learning_rate=0.0001, regularizers_l2=0, early_stop=False, device='cpu'):
    """
    ...
    X_train_np, y_train_np = load_dataset(pickle_path)
    ...
    # Transposer pour obtenir la forme (N, C, Z, X, Y) pour les Conv3D de PyTorch
    X_train_np = np.transpose(X_train_np, (0, 4, 1, 2, 3))
    
    if data_augmentation:
        X_train_augmented, y_train_augmented = data_augmentation_function_median_flip(X_train_np, y_train_np, working_dir=working_dir)
        print(f"Shape of X_train after augmentation: {X_train_augmented.shape}")
    else:
        X_train_augmented = X_train_np
        y_train_augmented = y_train_np
        print(f"Shape of X_train without augmentation: {X_train_augmented.shape}")

    # Division stratifiée train/test
    train_index, test_index = stratified_shuffle_split(X_train_augmented, y_train_augmented, test_size=0.2, random_state=random_state)
    X_train_balanced, X_test_balanced = X_train_augmented[train_index], X_train_augmented[test_index]
    ...
    mean = X_train_balanced.mean()
    std = X_train_balanced.std()

    train_transform = transforms.Compose([transforms.Normalize(mean, std)])
    val_transform = transforms.Compose([transforms.Normalize(mean, std)])

    # Création des Datasets et DataLoaders PyTorch
    train_dataset = CustomDataset(X_train_balanced, y_train_balanced, transform=train_transform)
    val_dataset = CustomDataset(X_test_balanced, y_test_balanced, transform=val_transform)
    train_loader = DataLoader(train_dataset, batch_size=batch_size, shuffle=True)
    val_loader = DataLoader(val_dataset, batch_size=batch_size, shuffle=False)

    # Vérification de la forme des données avant le modèle
    print(f"Shape of batch before model instantiation: {next(iter(train_loader))['image'].shape}")

    input_channels = next(iter(train_loader))['image'].shape[1]
    model = model_function(input_channels, dropout_rate=dropout).to(device)

    model_trained, history, y_pred, y_true = build_and_train_model(train_loader, val_loader, model, epochs=epochs, learning_rate=learning_rate, loss_fn=loss_fn, regularizers_l2=regularizers_l2, metric_list=metric_list, early_stop=early_stop, device=device)
    
    performance_visualizer(history, pickle_path, model_name, epochs, y_true, y_pred, working_dir)
    
    if performance_csv_path:
    	performances_saving(performance_csv_path, history, model_name, data_augmentation, batch_size, learning_rate, dropout, regularizers_l2, epochs)

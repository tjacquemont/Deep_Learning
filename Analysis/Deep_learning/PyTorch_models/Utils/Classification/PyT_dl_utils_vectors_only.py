...
class CustomDataset(Dataset):
    ...
    def __init__(self, X_vec, y, transform_img=None, transform_vec=None):
        """
        Args:
    ...
    def __getitem__(self, idx):
        sample = {'vector': self.X_vec[idx], 'label' :self.y[idx]}
...
def build_and_train_model(train_loader, val_loader, model, epochs=50, learning_rate=0.001, num_classes=1, regularizers_l2=0, metric_list=['accuracy'], early_stop = False, patience=None, device='cpu', work_dir='/home/thomas.jacquemont/cenir_thomas.jacquemont/PREP_AVC/Results/Deep_Learning/Debugs'):
    ...
    for epoch in range(epochs):
        ...
        for batch in train_loader:
            ...
            train_loss += loss.item() * inputs_vec.size(0)
        ...
        with torch.no_grad():
            for batch in val_loader:
                ...
                val_loss += loss.item() * inputs_vec.size(0)
...
def training_pipeline(pickle_path, model_function, model_name, working_dir, num_classes=1,
                      ...
                      dataset_index_save=True, model_save=True, device='cpu'):
    """
    ...
    # --- 2. Data augmentation not available
    X_vectors_train_augmented = X_vectors_train_np
    y_train_augmented = y_train_np
    print(f"Shape of X_train without augmentation: {X_vectors_train_augmented.shape}")
    ...
    transform_vec = lambda x: (x - torch.tensor(mean_vec)) / torch.tensor(std_vec)

    # --- 5. DataLoaders
    train_dataset = CustomDataset(X_vectors_train_balanced, y_train_balanced, transform_vec=transform_vec)
    val_dataset = CustomDataset(X_vectors_test_balanced, y_test_balanced, transform_vec=transform_vec)
    ...
    model = model_function(vector_features_nb, dropout_rate=dropout, num_classes=num_classes).to(device)
    ...
    # --- 9. Enregistrement des performances
    if perf_save:
